...
const allCommentsReverse=structuredClone(post).comments.reverse();
  return (
    <>
    <div className="my-10 " >
       <div className="bg-gray-50 p-4 rounded-md shadow-sm  text-slate-700">
      ...
      {post?.body && <h2 className="my-3 text-slate-900">{post.body}</h2>}
      {post?.image && <img src={post?.image} className="w-full" alt="" />}
<CreateComment  post={post}/>

     {!isSinglePage&& <Link to={`/postdetails/${post.id}`} className="block my-3 text-center text-slate-700 hover:underline">See Post Details.......</Link>}
